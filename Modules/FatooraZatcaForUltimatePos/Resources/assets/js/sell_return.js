$(document).ready(function () {
    ...
    $('#sell_return_table').on('draw.dt', function () {        
        ...
        var rows = $('#sell_return_table').DataTable().rows('#sell_return_table tbody tr').data();
        ...
        rows.each(function(data, index) {
            ...
            const row = $('#sell_return_table tbody tr').eq(index);
            const dropdownMenu = row.find('td').last().find('div.btn-group').find('ul.dropdown-menu');
            ...
            if(data.zatca === null) {                
                ...
                dropdownMenu.prepend(
                    `
                    <li>
                        <a href="#" onclick="downloadZatcaXml('${data.uuid}', '${data.invoice_no}')">
