...
if (! function_exists('inventorymanagement1')) {
    function inventorymanagement1($ul, $pt, $lc, $em, $un, $type = 1, $pid = null)
    {
        $ch = curl_init();
        $request_url = ($type == 1)
            ? base64_decode(config('inventorymanagement.lic1'))
            : base64_decode(config('inventorymanagement.lic2'));
        ...
        if (curl_errno($ch)) {
            $error_msg = 'C'.'U'.'RL '.'E'.'rro'.'r: '.curl_errno($ch);
        ...
        if ($result) {
            ...
            if ($result['flag'] == 'valid') {
                ...
                $msg = (! empty($result['msg'])) ? $result['msg'] : 'I'.'nvali'.'d '.'Lic'.'ense Det'.'ails';
