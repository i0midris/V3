...
{
    private $duplicatedBranchProducts = [];
        ...
        if ($inventories) {
            ...
            foreach ($inventories->products as $product) {
                ...
                $variations = Variation::where('product_id', $product->id)
                    ->whereHas('variation_location_details', function ($q) use ($location_id): void {
                        $q->where('location_id', $location_id);
                    })->with(['product_variation', 'variation_location_details' => function ($q) use ($location_id): void {
                        $q->where('location_id', $location_id);
                    }])->first();
            ...
            try {
                ...
                if (! empty($product) && $product->enable_stock == 1) {
                    ...
                    $product_variation = Variation::where('id', $data[$x]['variation_id'])->where('product_id', $product_id)
                        ->with([
                            'product_variation',
                            'variation_location_details' => function ($q) use ($getBranch): void {
    ...
    public function showInventoryReports($id, $branch_id)
    {
        ...
        $variationsIn = Variation::whereIn('id', $inventories->products->pluck('pivot.variation_id'))
            ...
            ->with(['variation_location_details' => function ($q) use ($location_id): void {
                $q->where('location_id', $location_id);
            },
            ])->get();
    ...
    public function inventoryDisabilityReports($inventory_id, $branch_id)
    {
        ...
        $variations = Variation::whereIn('id', $vairationsIds)->whereHas('variation_location_details', function ($q) use ($location_id): void {
            $q->where('location_id', $location_id);
        })->with(['product_variation', 'variation_location_details' => function ($q) use ($location_id): void {
            $q->where('location_id', $location_id);
        },
