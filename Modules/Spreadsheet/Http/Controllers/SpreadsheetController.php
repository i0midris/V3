...
class SpreadsheetController extends Controller
{
    ...
    public function index()
    {
        ...
        $todo_ids = isset($module_data['Essentials']) ? $module_data['Essentials'] : null;
        ...
        if (! auth()->user()->can('superadmin')) {
            $query->where('created_by', auth()->user()->id)
                ->orWhere(function ($query) use ($role_id): void {
                    $query->where('SSS.shared_id', '=', $role_id)
                        ->where('SSS.shared_with', '=', 'role');
                })
                ->orwhere(function ($query) use ($user_id): void {
                    $query->where('SSS.shared_id', '=', $user_id)
                        ->where('SSS.shared_with', '=', 'user');
                });

            if (! empty($todo_ids)) {
                $query->orwhere(function ($query) use ($todo_ids): void {
    ...
    public function show($id)
    {
        ...
        $query = Spreadsheet::where('business_id', $business_id)
            ...
            ->leftJoin('sheet_spreadsheet_shares as SSS', function ($join) use ($id): void {
