<div class="col-sm-6 col-lg-4 tw-mb-5 {{ $package->interval }} tw-relative">
    <div
        class="tw-flex tw-flex-col tw-gap-6 tw-p-6 box box-custom tw-transition-all tw-duration-700 hover:tw-scale-105 tw-cursor-pointer">

        @if ($package->mark_package_as_popular == 1)
            <div class="tw-dw-badge tw-dw-badge-custom tw-self-center tw-badge-lg">
        ...
        <div class="tw-flex tw-flex-col tw-text-center">
            ...
            <h3 class="subscription-title tw-inline-block tw-text-transparent tw-bg-clip-text hover:tw-text-[#467BF5] tw-font-semibold tw-text-2xl md:tw-text-3xl">
                @php
            ...
            <span class="tw-text-sm tw-text-gray-700 tw-font-semibold">{{ $package->description }}</span>
        </div>

        <!-- Features -->
        <div class="tw-flex tw-flex-col tw-text-white">
            <div class="tw-flex tw-gap-2 tw-items-center tw-text-[#1f1f1f] tw-truncate tw-font-semibold">
                <i class="fa fa-check subscription-title"></i>
        ...
        @else
            @if (isset($action_type) && $action_type == 'register')
                <a href="{{ route('business.getRegister') }}?package={{ $package->id }}"
                    class="add-btn tw-w-full tw-text-xl">
                    @if ($package->price != 0)
