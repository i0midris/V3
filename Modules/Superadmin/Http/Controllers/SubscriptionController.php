            ...
            if ($request->has('code')) {
                ...
                if ($coupon) {
                    ...
                    if (($coupon->is_active == 1) && ((is_array($package_ids) && in_array($package_id, $package_ids)) || is_null($coupon->applied_on_packages)) && ((is_array($business_ids) && in_array($business_id, $business_ids)) || is_null($coupon->applied_on_business)) && (Carbon::parse($coupon->expiry_date)->greaterThanOrEqualTo($current_date) || is_null($coupon->expiry_date))) {
                        ...
                        elseif ((is_array($package_ids) && ! in_array($package_id, $package_ids)) && ! is_null($coupon->applied_on_packages)) {

                            $coupon_status = ['status' => 'danger', 'msg' => __('superadmin::lang.coupon_not_matched_with_package')];
                        }
                        // check coupon with this business
                        elseif ((is_array($business_ids) && ! in_array($business_id, $business_ids)) && ! is_null($coupon->applied_on_business)) {

                            $coupon_status = ['status' => 'danger', 'msg' => __('superadmin::lang.coupon_not_matched_with_business')];
                        }
                        //  check expiry date
                        elseif (Carbon::parse($current_date)->greaterThanOrEqualTo($coupon->expiry_date) && ! is_null($coupon->expiry_date)) {
    ...
    public function allSubscriptions()
    {
        ...
        return Datatables::of($subscriptions)
            ...
            ->addColumn('action', function ($row) {
                return '<button type="button" class="custom-tbtn tw-dw-btn-outline  tw-dw-btn-primary btn-modal" data-container=".view_modal" data-href="'.action([\Modules\Superadmin\Http\Controllers\SubscriptionController::class, 'show'], $row->id).'" ><i class="fa fa-eye" aria-hidden="true"></i> '.__('messages.view').'</button>';
            })
