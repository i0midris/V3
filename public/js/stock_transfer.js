...
function update_table_total() {
    ...
    $('#stock_adjustment_product_table tbody tr').each(function () {
        const row = $(this);
        const qty_input = row.find('input.product_quantity');
        const line_total_input = row.find('input.product_line_total');

        const quantity = parseFloat(__read_number(qty_input)) || 0;

        let multiplier = 1;
        const sub_unit = row.find('select.sub_unit');
        if (sub_unit.length) {
            const selected = sub_unit.find(':selected');
            multiplier = parseFloat(selected.data('multiplier')) || 1;
        }

        total_qty += quantity * multiplier;

        const this_total = parseFloat(__read_number(line_total_input)) || 0;
        table_total += this_total;
    });

    $('#qty_adjustment').text(__number_f(total_qty));
    $('#total_adjustment').text(__number_f(table_total));

    const shipping_input = $('#shipping_charges');
    if (shipping_input.length) {
        const shipping = parseFloat(__read_number(shipping_input)) || 0;
        table_total += shipping;
    }

    $('#final_total_text').text(__number_f(table_total));
    $('#total_amount').val(table_total);
}
