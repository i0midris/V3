...
function __currency_trans_from_en(
    ...
    if (
        ...
        s = typeof __currency_symbol !== 'undefined' ? __currency_symbol : '$';
        thousand = typeof __currency_thousand_separator !== 'undefined' ? __currency_thousand_separator : ',';
        decimal = typeof __currency_decimal_separator !== 'undefined' ? __currency_decimal_separator : '.';
    }
    ...
    if (show_symbol) {
        symbol = s;
        format = '%s %v';
        if (typeof __currency_symbol_placement !== 'undefined' && __currency_symbol_placement === 'after') {
            format = '%v %s';
        }
    }

    if (is_quantity) {
        precision = typeof __quantity_precision !== 'undefined' ? __quantity_precision : 2;
        show_symbol = false;
    }

    // Apply exchange rate if currency_id is present
    let rate = 1;
    if ($('#current_currency_rate').length > 0) {
        rate = parseFloat($('#current_currency_rate').val()) || 1;
    }

    input = parseFloat(input) * rate;

    return accounting.formatMoney(input, symbol, precision, thousand, decimal, format);
}


function convert_all_currency() {
    const exchange_rate = parseFloat($('#current_currency_rate').val()) || 1;

    $('#pos_table tbody tr').each(function () {
        const $row = $(this);

        // Get the price in base currency (e.g., YER)
        const base_unit_price = parseFloat($row.find('.base-unit-price').val()) || 0;

        // Convert from base to foreign currency (e.g., YER to SAR)
        const converted_price = base_unit_price / exchange_rate;

        // Update price inputs and labels
        $row.find('.pos_unit_price_inc_tax').val(converted_price.toFixed(__currency_precision));
        $row.find('.pos_unit_price_inc_tax_text')
            .text(converted_price.toFixed(__currency_precision))
            .data('orig-value', converted_price.toFixed(__currency_precision));

        // Update subtotal based on converted price
        const quantity = parseFloat($row.find('.pos_quantity').val()) || 1;
        const subtotal = quantity * converted_price;

        $row.find('.row_subtotal')
            .text(subtotal.toFixed(__currency_precision))
            .data('orig-value', subtotal.toFixed(__currency_precision));

        $row.find('.row_subtotal_hidden').val(subtotal.toFixed(__currency_precision));

        // Trigger for any bound logic
        $row.find('.pos_unit_price_inc_tax').trigger('change');
    });

    // Recalculate full totals and reformat
    pos_total_row();
    __currency_convert_recursively($('#pos_table'), true);
}
...
function __currency_convert_recursively(element, use_page_currency = false) {
    element.find('.display_currency').each(function() {
        let original_value = $(this).data('orig-value');
        if (typeof original_value === 'undefined') {
            original_value = __number_uf($(this).text());
        }

        let show_symbol = $(this).data('currency_symbol') === true;
        let is_quantity = $(this).data('is_quantity') === true;

        if (typeof $(this).data('use_page_currency') !== 'undefined') {
            use_page_currency = $(this).data('use_page_currency');
        }

        let highlight = $(this).data('highlight') === true;
        if (highlight) {
            __highlight(original_value, $(this));
        }

        let formatted = __currency_trans_from_en(original_value, show_symbol, use_page_currency, __currency_precision, is_quantity);
        $(this).text(formatted);
    });
}
