$(document).ready(function() {
    ...
    $(document).on('click', 'button.delete_brand_button', function() {
        const button = $(this); // to retain `this` reference inside promise
    ...
    $(document).on('click', 'button.delete_unit_button', function() {
        const button = $(this); // preserve reference to `this`
                        ...
                        success: function(result) {
                            if (result.is_mobile_exists === true) {
                                Swal.fire({
                                    ...
                                    icon: 'warning',
                                    showCancelButton: true,
                                    confirmButtonText: LANG.ok || 'Continue',
    ...
    $(document).on('click', 'a.delete_contact_button', function (e) {
        e.preventDefault();
        ...
        Swal.fire({
            ...
            text: LANG.confirm_delete_contact || 'You won’t be able to revert this!',
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d33',
            cancelButtonColor: '#3085d6',
            confirmButtonText: LANG.ok || 'Yes, delete it!',
            cancelButtonText: LANG.cancel || 'Cancel',
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            ...
                            toastr.error(response.msg || 'Failed to delete contact.');
                        }
                    },
                    error: function (xhr) {
    ...
    $(document).on('click', 'button.delete_variation_button', function() {
        const button = $(this); // preserve reference to avoid `this` issues inside promise
    ...
    $(document).on('click', 'button.delete_tax_group_button', function() {
        const button = $(this); // Capture 'this' to use inside AJAX
    ...
    $(document).on('click', 'button.delete_invoice_button', function() {
        const button = $(this); // store `this` context safely
    ...
    business_locations = $('#business_location_table').DataTable({
        ...
        fixedHeader:false,
        dom: `
            <"tw-mt-4 dt-section-toolbar tw-mb-4 tw-flex tw-flex-wrap tw-justify-end tw-items-center tw-gap-2"f>
            rt
            <"tw-mt-4 tw-flex tw-justify-end tw-items-center"i>
        `,
        buttons: [],
    ...
    $(document).on('click', 'a.delete_expense', function(e) {
        e.preventDefault();
        const link = $(this); // capture reference
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const data = link.serialize(); // usually not needed for `<a>`, can be removed
    ...
    $(document).on('click', 'button.delete_commsn_agnt_button', function() {
        const button = $(this); // retain reference to `this`
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                const href = button.data('href');
                const data = button.serialize(); // if needed; can be replaced with {} if empty
    ...
    $(document).on('click', 'button.delete_customer_group_button', function() {
        const button = $(this); // keep reference
    ...
    $(document).on('click', '.delete-sale', function(e) {
        ...
        Swal.fire({
            ...
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: LANG.ok,  // Optional: if you have a localized OK
...
$(document).on('click', 'button.delete_discount_button', function() {
    const button = $(this); // retain reference to avoid context loss
...
$(document).on('click', 'a.delete_purchase_return', function(e) {
    e.preventDefault();
    const link = $(this); // retain reference to avoid context issues
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const data = {}; // serialize() on <a> returns nothing, so safely use empty data
...
$(document).on('click', 'button.activate-deactivate-location', function() {
    const button = $(this); // preserve reference to this element
...
$(document).on('click', '.delete-media', function () {
    const button = $(this); // retain reference to avoid `this` context loss
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const thumbnail = button.closest('.img-thumbnail');
...
function submitContactForm(form) {
    ...
    $submitBtn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> ' + LANG.saving);
    ...
    $.ajax({
        ...
        error: function() {
            // Handle server error (500, timeout, etc.)
            toastr.error('Something went wrong. Please try again.');
            resetFormSubmissionState();
        }
    });
