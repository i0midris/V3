...
if ($('#search_product').length) {
    ...
    input.on('keydown', function (e) {
        // Barcode scanner sends 'Enter' after full code
        if (e.key === 'Enter' || e.keyCode === 13) {
            e.preventDefault(); // Prevent form submit or accidental reload
            ...
            $.getJSON('/products/list', {
                ...
                if (data.length === 1 && canAddProduct(data[0])) {
                    const item = data[0];
                    const purchase_line_id = item.purchase_line_id && term === item.lot_number
    ...
    // Autocomplete for manual input
    input.autocomplete({
        delay: 200,
        minLength: 2,
        source: function (request, response) {
            const search_fields = $('.search_fields:checked').map(function () {
                return $(this).val();
            }).get();
        ...
        const price = item.variation_group_price || item.selling_price;
        const price_display = __currency_trans_from_en(price, false, false, __currency_precision, true);
        const nameLine = `${item.name}${item.type === 'variable' ? ' - ' + item.variation : ''}`;
        const subSku = `(${item.sub_sku})`;
        let html = `<div>${nameLine} ${subSku}<br>Price: ${price_display}`;
    ...
    function canAddProduct(item) {
        const overselling = $('input#is_overselling_allowed').length > 0;
        const is_so = $('#sale_type').val() === 'sales_order';
        const is_draft = ['quotation', 'draft'].includes($('input#status').val());
        return (
            item.enable_stock !== 1 ||
    ...
    $(document).on('click', '.remove_payment_row', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                button.closest('.payment_row').remove();
        ...
        .validate({
            ...
            submitHandler: function(form) {
                $.ajax({
                    ...
                    success: function(result) {
                        // No condition, just submit the form regardless of mobile existence
...
function updateCurrencyRate(code, rate, symbol) {
    $('#exchange_rate').val(rate);
    $('#current_currency_rate').val(rate);
    $('#current_currency_symbol').val(symbol || '');
    $('#current_currency_code').val(code || '');

    convert_all_currency(); // Converts all row prices to new currency
    pos_total_row();        // Recalculates subtotal, tax, total, etc.
}

// Handle dropdown where currency data (rate & symbol) is embedded in <option>
$('#currency_id').on('change', function () {
    ...
    updateCurrencyRate(code, rate, symbol);
});

// Handle dropdown where only the code is selected, and rate is fetched via AJAX
$('#currency_code').on('change', function () {
    const code = $(this).val();
    
    if (code) {
        $.get(`/currency-rates/get-rate/${code}`, function (data) {
