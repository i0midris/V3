$(document).ready(function() {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                d.min_days_ago = $('#min_days_ago').val();
                d.max_days_ago = $('#max_days_ago').val();
        ...
        columns: [
            ...
            { data: 'due', name: 'due' },
            {
                ...
                render: function (data, type, row) {
                    if (type === 'sort') {
                        let match = data.match(/data-order\s*=\s*["']?(\d+)["']?/);
    ...
    $('#min_days_ago, #max_days_ago').on('input', function () {
        supplier_report_tbl.ajax.reload();
        });
    ...
    { data: 'currency_code' },
        ...
        sr_sales_report = $('table#sr_sales_report').DataTable({
            ...
            columns: [
                ...
                { data: 'invoice_age', name: 'created_at', searchable: false } // <-- NEW
            ],
            columnDefs: [
                {
                    searchable: false,
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                d.currency_code = $('select#currency_code').val();
                var start = '';
                var end = '';
        ...
        columns: [
            ...
            { data: 'method', name: 'method' },
            { data: 'invoice_no', name: 't.invoice_no' },
            { data: 'base_final_total', name: 't.base_final_total' },
    { data: 'currency_code', name: 't.currency_code' }, // NEW 
            { data: 'action', orderable: false, searchable: false },
        ],
        fnDrawCallback: function(oSettings) {
            var total_amount = sum_table_col($('#sell_payment_report_table'), 'paid-amount');
            $('#footer_total_amount').text(total_amount);
            var base_total = sum_table_col($('#sell_payment_report_table'), 'base_final_total');
$('#footer_base_total').text(base_total);
    ...
    $('#sell_payment_report_form #location_id, #sell_payment_report_form #customer_id, #sell_payment_report_form #payment_types, #sell_payment_report_form #customer_group_filter, #sell_payment_report_form #currency_code').change(
        function() {
            sell_payment_report.ajax.reload();
        }
    );    
