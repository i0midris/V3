    ...
    public function uf_date($date, $time = false)
{
    try {
        ...
        // Handle natural language like 'now', 'today', etc.
        if (in_array(strtolower($date), ['now', 'today', 'yesterday']) || strtotime($date) !== false) {
            return \Carbon\Carbon::parse($date)->format($mysql_format);
        }

        // Fallback: try parsing based on business format
    ...
    public function uf_time($time)
{
    try {
        ...
        // Try direct parse for natural strings like 'now'
        if (in_array(strtolower($time), ['now']) || strtotime($time) !== false) {
            return \Carbon\Carbon::parse($time)->format('H:i');
        }

        return \Carbon\Carbon::createFromFormat($time_format, $time)->format('H:i');
    } catch (\Exception $e) {
        return null;
    }
}
     ...
     * @return strin
     */
    public function format_date($date = 'now', $show_time = false, $business_details = null)
{
    ...
    if ($date === 'now' || empty($date)) {
        ...
        $date = \Carbon\Carbon::parse($date, $timezone)->timezone($timezone);
    } elseif ($date instanceof \DateTimeInterface) {
        $date = \Carbon\Carbon::instance($date)->timezone($timezone);
    } else {
        return null;
    }
    ...
    public function giveLocationPermissions($user, $request)
    {
        ...
        if (empty($permissions) && ! empty($location_permissions)) {
            ...
            if (is_array($permitted_locations)) {
                foreach ($permitted_locations as $value) {
