...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'currency_code' => !empty($input['currency_code']) ? $input['currency_code'] : null,
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                if (! empty($product['line_discount_amount'])) {
                    ...
                    if ($product['line_discount_type'] == 'fixed') {
                        $line_discount_amount /= $multiplier;
                    }
                }
...
'base_amount' => !empty($transaction->currency_code)
    ? ($payment_amount / $transaction->exchange_rate)
    : null,
                    ];
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['mobile_number'] = $location_details->mobile ?? '';
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                if (!empty($line['group_tax_details'])) {
                    ...
                    $tax_name = $line['tax_name'] ?? 'Tax';
                    if (!isset($output['taxes'][$tax_name])) {
                        $output['taxes'][$tax_name] = 0;
                    }
                    // You must ensure line has quantity_uf and tax_unformatted set
                    $output['taxes'][$tax_name] += ($line['tax_unformatted'] ?? 0) * ($line['quantity_uf'] ?? 1);
                }
            }
        }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if ($zatca_qr) {
                ...
                $qr_code_text = $this->_zatca_qr_text(
                    $transaction,
    ...
    protected function _receiptDetailsSellReturnLines($lines, $il, $business_details)
{
    ...
    foreach ($lines as $line) {
        ...
        $tax_details = \App\TaxRate::find($line->tax_id);
        ...
        $line_array = [
            ...
            'quantity_uf' => $line->quantity_returned, // 👈 Required for accurate tax summary
            'units' => $unit_name,
            ...
            'tax' => $this->num_f($line->item_tax ?? 0, false, $business_details),
            'tax_id' => $line->tax_id,
            'tax_name' => $tax_details->name ?? null,
            'tax_percent' => $tax_details->amount ?? 0,
            'tax_unformatted' => $line->item_tax ?? 0, // 👈 Required for accurate tax summary

            'line_discount' => 0,
        ];

        // Group tax details
        if (!empty($tax_details) && $tax_details->is_tax_group) {
        ...
        // Optional: Add lot number or expiry if enabled
        if ($is_lot_number_enabled && $il->show_lot == 1) {
            $line_array['lot_number'] = $line->lot_details->lot_number ?? null;
        }

        if ($is_product_expiry_enabled && $il->show_expiry == 1) {
            $line_array['product_expiry'] = !empty($line->lot_details->exp_date)
                ? $this->format_date($line->lot_details->exp_date, false, $business_details)
                : null;
        }
    ...
    {
        $query = Transaction::leftjoin('transactions as SR', function ($join): void {
    ...
    public function payAtOnce($parent_payment, $type)
{
        ...
        foreach ($due_transactions as $transaction) {
            ...
            if ($total_amount > 0) {
                ...
                $array = [
                    ...
                    // ✅ Add currency fields inside the loop
                    'base_amount' => $parent_payment->base_amount,
                    'currency_code' => $parent_payment->currency_code,
                    'exchange_rate' => $parent_payment->exchange_rate,
                ];

                $prefix_type = in_array($transaction->type, ['sell', 'sell_return'])
                    ? 'sell_payment'
                    : 'purchase_payment';

                $ref_count = $this->setAndGetReferenceCount($prefix_type, $parent_payment->business_id);
                $array['payment_ref_no'] = $this->generateReferenceNumber($prefix_type, $ref_count, $parent_payment->business_id);

                if ($due <= $total_amount) {
                    $array['amount'] = $due;
                    TransactionPayment::create($array);
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        if (!empty($business['pos_settings']) && !is_array($business['pos_settings'])) {
            $decoded = json_decode($business['pos_settings'], true);
            $business['pos_settings'] = json_last_error() === JSON_ERROR_NONE ? $decoded : [];
        }
        ...
        if ($check_expiry) {
            ...
            if (
                ($businessSession['enable_product_expiry'] ?? null) == 1 &&
                ($businessSession['on_product_expiry'] ?? null) === 'stop_selling' &&
        ...
        foreach ($transaction_lines as $line) {
            ...
            if ($stop_selling_expired && empty($purchase_line_id)) {
                ...
                $query->where(function ($q) use ($expiry_date): void {
            ...
            foreach ($rows as $row) {
                ...
                if ($qty_selling <= $row->quantity_available) {
                    ...
                    $qty_selling -= $row->quantity_available;
    ...
    public function adjustMappingPurchaseSell(
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            ...
            if (! empty($deleted_line_ids)) {
                ...
                $sell_purchases += $deleted_sell_purchases;
            }
                ...
                if (empty($line->slpl_id)) {
                    ...
                    if ($total_sold_entry->quantity != $line->quantity) {
                        if ($line->quantity > $total_sold_entry->quantity) {
                            ...
                            $line_temp = $line;
                            $line_temp->quantity -= $total_sold_entry->quantity;
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [], $permitted_locations = null)
    {
        ...
        if ($is_opening) {
            ...
            $query->where(function ($query) use ($date, $next_day): void {
    ...
    public function recalculateSellLineTotals($business_id, $sell_line)
    {
        ...
        if (! empty($sub_unit)) {
            ...
            if ($sell_line->line_discount_type == 'fixed') {
                $sell_line->line_discount_amount *= $multiplier;
            }
            $sell_line->orig_quantity = $sell_line->quantity;
            $sell_line->multiplier = $multiplier;
            $sell_line->quantity /= $multiplier;
            $sell_line->unit_price_before_discount *= $multiplier;
            $sell_line->unit_price *= $multiplier;
            $sell_line->unit_price_inc_tax *= $multiplier;
            $sell_line->item_tax *= $multiplier;
            $sell_line->quantity_returned /= $multiplier;
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        $transaction_query = $this->__transactionQuery($contact_id, $start, $end, $location_id)
            ->with(['location'])
            ->select('transactions.*', 'transactions.currency_code', 'transactions.base_final_total');
        ...
        foreach ($transactions as $transaction) {
            ...
            $temp_array = [
                ...
                'transaction_id' => $transaction->id,
                'transaction_type' => $transaction->type,
                'base_final_total' => $transaction->base_final_total ?? 0,
                'currency_code' => $transaction->currency_code ?? '',
            ];
    ...
    private function __paymentQuery($contact_id, $start, $end = null, $location_id = null)
    {
        ...
        if (! empty($location_id)) {
            ...
            $query->where(function ($q) use ($location_id): void {
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $inputs = $request->only(['amount', 'method', 'note', 'card_number', 'card_holder_name',
            ...
            'cheque_number', 'bank_account_number','transaction_id', 'account_id' ]);
        ...
        if ($format_data && is_string($inputs['paid_on'])) {
        ...
        if ($request->has('base_amount')) {
    $inputs['base_amount'] = $this->num_uf($request->input('base_amount'));
}
if ($request->has('currency_code')) {
    $inputs['currency_code'] = $request->input('currency_code');
}
if ($request->has('exchange_rate')) {
    $inputs['exchange_rate'] = $this->num_uf($request->input('exchange_rate'));
}
...
$invoice_total = $productUtil->calculateInvoiceTotal(
    ...
    foreach ($input['products'] as $product_line) {
        ...
        $quantity_before = $sell_line->quantity_returned ?? 0;
        $total_returned = $quantity_before + $new_return_qty;

        if ($total_returned > $sell_line->quantity) {
            throw new \Exception('Return quantity exceeds sold quantity for product: ' . $sell_line->product->name);
        }

        $sell_line->quantity_returned = $total_returned;
        $sell_line->save();
...
public function payCustomerForReturn($transaction, $business_id, $user_id)
{
    $parent_transaction = Transaction::find($transaction->return_parent_id);
    $status = $parent_transaction->payment_status ?? 'unknown';
    ...
    try {
        ...
        $request_data->merge([
            ...
            'is_reverse'         => 1,
            'note'               => 'Auto refund for sell return #' . $transaction->invoice_no,
            'transaction_id'     => $transaction->id,
            'account_id'         => $transaction->account_id ?? null,
        ]);

        \Log::info("Attempting auto refund for sell return ID {$transaction->id}", [
            'invoice_no'        => $transaction->invoice_no,
            'refunded_so_far'   => $already_refunded,
            'to_refund'         => $to_refund,
            'account_id'        => $transaction->account_id ?? 'null',
            'paid_on'           => now()->toDateTimeString(),
        ]);

        $this->payContact($request_data, true);

        \Log::info("Refund of {$to_refund} successfully recorded for Sell Return #{$transaction->invoice_no} (ID: {$transaction->id})");

    } catch (\Exception $e) {
        \Log::error("Auto refund failed for Sell Return #{$transaction->invoice_no} (ID: {$transaction->id}): " . $e->getMessage(), [
            'line' => $e->getLine(),
            'file' => $e->getFile(),
