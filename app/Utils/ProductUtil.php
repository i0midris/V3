...
class ProductUtil extends Util
{
    ...
    public function calculateComboQuantity($location_id, $combo_variations)
    {
        ...
        foreach ($combo_variations as $key => $value) {
            $variation = Variation::with(['product', 'variation_location_details' => function ($q) use ($location_id): void {
    ...
    public function calculateComboDetails($location_id, $combo_variations)
    {
        ...
        foreach ($combo_variations as $value) {
            $variation = Variation::with(['product', 'variation_location_details' => function ($q) use ($location_id): void {
                $q->where('location_id', $location_id);
            }])->findOrFail($value['variation_id']);
    ...
    public function changePurchaseLineUnit($purchase_line, $business_id)
    {
        ...
        if (! empty($sub_unit)) {
            ...
            $purchase_line->quantity /= $multiplier;
            $purchase_line->pp_without_discount *= $multiplier;
            $purchase_line->purchase_price *= $multiplier;
            $purchase_line->purchase_price_inc_tax *= $multiplier;
            $purchase_line->item_tax *= $multiplier;
            $purchase_line->quantity_returned /= $multiplier;
            $purchase_line->quantity_sold /= $multiplier;
            $purchase_line->quantity_adjusted /= $multiplier;
        }
    ...
    public function changeSellLineUnit($business_id, $sell_line)
    {
        ...
        if (! empty($sub_unit)) {
            ...
            $sell_line->quantity_ordered /= $multiplier;
            $sell_line->item_tax *= $multiplier;
            $sell_line->default_sell_price *= $multiplier;
            $sell_line->unit_price_before_discount *= $multiplier;
            $sell_line->sell_price_inc_tax *= $multiplier;
    ...
    public function getProductDiscount($product, $business_id, $location_id, $is_cg = false, $price_group = null, $variation_id = null)
    {
        ...
        $query = Discount::where('business_id', $business_id)
            ...
            ->where('starts_at', '<=', $now)
            ->where('ends_at', '>=', $now)
            ->where(function ($q) use ($product, $variation_id): void {
                $q->where(function ($sub_q) use ($product): void {
                    ...
                    ->orWhere(function ($sub_q) use ($product): void {
                ...
                if (! empty($variation_id)) {
                    $q->orWhereHas('variations', function ($sub_q) use ($variation_id): void {
        ...
        if (! is_null($price_group)) {
            $query->where(function ($q) use ($price_group): void {
                $q->whereNull('spg')
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        if (! empty($price_group_id)) {
            $query->leftjoin(
                ...
                function ($join) use ($price_group_id): void {
        ...
        if (! empty($search_term)) {
            ...
            if ($search_type == 'like') {
                $query->where(function ($query) use ($search_term, $search_fields): void {
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        $stock_history = Transaction::leftjoin('transaction_sell_lines as sl',
            ...
            ->where(function ($q) use ($variation_id): void {
