    ...
    public function getRegisterDetails($register_id = null)
{
    ...
    // ✅ Append transaction_payments-based summary
    if ($register_details) {
        $end_time = $register_details->closed_at ?? now();
        $payments = DB::table('transaction_payments')
            ->where('created_by', $register_details->user_id)
            ->where('business_id', auth()->user()->business_id)
            ->whereBetween('created_at', [$register_details->open_time, $end_time])
            ->where('payment_type', 'credit')
            ->whereNotNull('method')
            ->whereNotNull('amount')
            ->selectRaw("
                SUM(CASE WHEN method = 'cash' THEN amount ELSE 0 END) as tp_total_cash,
                SUM(CASE WHEN method = 'card' THEN amount ELSE 0 END) as tp_total_card,
                SUM(CASE WHEN method = 'cheque' THEN amount ELSE 0 END) as tp_total_cheque,
                SUM(CASE WHEN method = 'bank_transfer' THEN amount ELSE 0 END) as tp_total_bank_transfer,
                SUM(CASE WHEN method = 'other' THEN amount ELSE 0 END) as tp_total_other,
                COUNT(CASE WHEN method = 'card' THEN 1 END) as tp_total_card_slips,
                COUNT(CASE WHEN method = 'cheque' THEN 1 END) as tp_total_cheques
            ")
            ->first();

        foreach ((array)$payments as $key => $value) {
