                ...
                foreach ($opening_stocks as $location_id => $value) {
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        foreach ($value as $vid => $purchase_lines_data) {
                            ...
                            foreach ($purchase_lines_data as $pl) {
                                ...
                                if (isset($pl['purchase_line_id'])) {
                                    ...
                                    $qty_remaining += $purchase_line->quantity_used;
                        ...
                        if (count($delete_transactions) > 0) {
                            foreach ($delete_transactions as $delete_transaction) {
                                ...
                                $delete_transaction_data[] = $delete_transaction->id;
                        ...
                        if (! empty($new_purchase_lines)) {
                            foreach ($new_purchase_lines as $key => $new_purchase_line) {
                                ...
                                $new_transaction_data[$key]['purchase_line'] = $new_purchase_line;
                    ...
                    (new \App\Utils\ModuleUtil)->getModuleData('MKamel_store110', ['request' => $request, 'new_transaction_data' => $new_transaction_data, 'edit_transaction_data' => $edit_transaction_data, 'delete_transaction_data' => $delete_transaction_data, 'location_id' => $location_id, 'user_id' => $user_id]);
                }
