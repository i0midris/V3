...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        // prepare sells_chart_1 separate variables
        $sells_chart_1_labels = $labels;

        $sells_chart_1_options = $this->__chartOptions(
            __('home.total_sells', ['currency' => $currency->code])
        );

        $sells_chart_1_datasets = [];

        if (! empty($location_sells)) {
            foreach ($location_sells as $location_sell) {
                $sells_chart_1_datasets[] = [
                    'name' => $location_sell['loc_label'],
                    'type' => 'line',
                    'data' => $location_sell['values'],
                ];
            }
        }
        if (count($all_locations) > 1) {
            $sells_chart_1_datasets[] = [
                'name' => __('report.all_locations'),
                'type' => 'line',
                'data' => $all_sell_values,
            ];
        }

        // rebuild fiscal year chart
        $labels = [];
        $values = [];
        ...
        //trying something 
        $sells_chart_2_labels = $labels;

        // Prepare options by calling your existing method (assuming it returns an array)
        $sells_chart_2_options = $this->__chartOptions(
            __('home.total_sells', ['currency' => $currency->code])
        );

        // Prepare datasets as an array of arrays
        $sells_chart_2_datasets = [];

        // Add each location sell dataset
        if (!empty($fy_sells_by_location_data)) {
            foreach ($fy_sells_by_location_data as $location_sell) {
                $sells_chart_2_datasets[] = [
                    'name' => $location_sell['loc_label'],   // Dataset label
                    'type' => 'line',                        // Chart type
                    'data' => $location_sell['values'],     // Values array
                ];
            }
        }

        // Add all locations dataset if more than 1 location
        if (count($all_locations) > 1) {
            $sells_chart_2_datasets[] = [
                'name' => __('report.all_locations'),
                'type' => 'line',
        ...
        //  dd($sells_chart_1);
        return view('home.index', compact('sells_chart_1_labels', 'sells_chart_1_options', 'sells_chart_1_datasets', 'sells_chart_2_options', 'sells_chart_2_labels', 'sells_chart_2_datasets','sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin'));
    }
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            $purchase_details['purchase_due'] -= $total_ledger_discount['total_purchase_discount'];
