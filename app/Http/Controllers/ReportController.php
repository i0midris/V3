...
class ReportController extends Controller
{
    ...
    {
        if (! auth()->user()->can('view_location_report')) {
        ...
        if ($request->ajax()) {
            $location_ids = (array) $request->get('location_id');
            $start_date = $request->get('start_date');
            $end_date = $request->get('end_date');
            ...
            foreach ($location_ids as $location_id) {
                $query = DB::table('products as p')
                    ...
                    ->select(
                        ...
                        DB::raw("(
                            ...
                            WHERE t.location_id = vld.location_id
                              ...
                              AND t.status = 'final'
                              AND t.type = 'sell'
                              " . ($start_date && $end_date ? "AND DATE(t.transaction_date) BETWEEN '$start_date' AND '$end_date'" : "") . "
                        ) as total_sold"),
                        ...
                        DB::raw("(
                            ...
                            WHERE t.location_id = vld.location_id
                              AND sal.product_id = p.id
                              AND t.type = 'stock_adjustment'
                              " . ($start_date && $end_date ? "AND DATE(t.transaction_date) BETWEEN '$start_date' AND '$end_date'" : "") . "
                        ) as total_adjusted"),
    
                        DB::raw("(
                            SELECT COALESCE(SUM(pl.purchase_price_inc_tax * pl.quantity), 0)
                        ...
                        DB::raw("(
                            SELECT COALESCE(SUM(tsl.unit_price_inc_tax * tsl.quantity), 0)
                            FROM transaction_sell_lines tsl
                            JOIN transactions t ON t.id = tsl.transaction_id
                            WHERE t.location_id = vld.location_id
                              AND tsl.variation_id = v.id
                        ...
                        DB::raw("(
                            SELECT COALESCE(SUM(tsl.quantity_returned), 0)
                            FROM transaction_sell_lines tsl
                            JOIN transactions t ON t.id = tsl.transaction_id
                            WHERE t.location_id = vld.location_id 
                        ...
                        DB::raw("(
                            SELECT COALESCE(SUM(tsl.quantity_returned * tsl.unit_price_inc_tax), 0)
                            FROM transaction_sell_lines tsl
                            JOIN transactions t ON t.id = tsl.transaction_id
                            WHERE t.location_id = vld.location_id 
                              AND tsl.product_id = p.id
                    ...
                    ->groupBy('p.id', 'vld.location_id', 'p.name', 'l.name')
                    ->orderBy('p.name')
                    ->get();
        ...
        return view('report.location_summary')->with(compact('business_locations'));
        ...
        if ($request->ajax()) {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    ...
                    DB::raw("MAX(t.created_at) as last_transaction_date"),
                    DB::raw("(SELECT type FROM transactions 
                    WHERE transactions.contact_id = contacts.id 
                    AND transactions.created_at = MAX(t.created_at) 
            ...
            if (!empty($request->min_days_ago) || !empty($request->max_days_ago)) {
    if (!empty($request->min_days_ago)) {
        $contacts->havingRaw('DATEDIFF(CURDATE(), MAX(t.created_at)) >= ?', [$request->min_days_ago]);
    }
    if (!empty($request->max_days_ago)) {
        $contacts->havingRaw('DATEDIFF(CURDATE(), MAX(t.created_at)) <= ?', [$request->max_days_ago]);
    }
}
            ...
            if ($request->has('order')) {
                ...
                $order_column_name = $request->input("columns.$order_column_index.data");
                $order_direction = $request->input('order.0.dir');
            
                if ($order_column_name === 'last_transaction') {
                    $contacts->orderByRaw("MAX(t.created_at) $order_direction");
                } else {
                    // Optional: you can add more orderable column logic here
                    $contacts->orderBy('contacts.id', 'desc'); // fallback
                }
            }
            ...
            return Datatables::of($contacts)
                ...
                ->addColumn('last_transaction', function ($row) {
                    if (!empty($row->last_transaction_date)) {
                        \Carbon\Carbon::setLocale('ar');
                        $last_date = \Carbon\Carbon::parse($row->last_transaction_date);
                        $time_diff = $last_date->diffForHumans(null, [
                        ...
                        return '<span class="last_transaction" data-order="' . strtotime($row->last_transaction_date) . '">' .
    $time_diff . ' - ' . $type_ar .
'</span>';
                ...
                ->rawColumns(['total_purchase', 'total_invoice', 'due', 'name', 'total_purchase_return', 'total_sell_return', 'opening_balance_due','last_transaction' ])
                ->make(true);
        }
        ...
        if ($request->ajax()) {
            ...
            $datatable = Datatables::of($products)
                ...
                ->addColumn('action', function ($row) {
                    return '<a class="custom-tbtn tw-dw-btn-outline  tw-dw-btn-info tw-w-max " href="'.action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$row->product_id]).
                ...
                ->filterColumn('variation', function ($query, $keyword): void {
        ...
        if ($request->ajax()) {
            ...
            if ($type == 'purchase') {
                $sells->where('transactions.type', 'purchase')
                    ...
                    ->where(function ($query): void {
                        $query->whereHas('purchase_lines', function ($q): void {
                            $q->whereNotNull('purchase_lines.tax_id');
                        })->orWhereNotNull('transactions.tax_id');
                    })
                    ->with(['purchase_lines' => function ($q): void {
                        $q->whereNotNull('purchase_lines.tax_id');
    ...
    public function getTaxReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            foreach ($module_output_taxes as $module_output_tax) {
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($registers)
                ...
                ->addColumn('action', '<button type="button" data-href="{{action(\'App\Http\Controllers\CashRegisterController@show\', [$id])}}" class="custom-tbtn tw-dw-btn-outline  tw-dw-btn-info tw-w-max btn-modal" 
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id): void {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['purchase', 'opening_balance']);
            })
                ->where('transaction_payments.business_id', $business_id)
                ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part): void {
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (! empty($request->get('currency_code'))) {
                $query->where('t.currency_code', $request->get('currency_code'));
            }
            ...
            return Datatables::of($query)
                ...
                ->editColumn('base_final_total', function ($row) {
                    // Format number ONLY, no symbol
                    return '<span class="base_final_total" data-orig-value="' . $row->base_final_total . '">' .
                        number_format($row->base_final_total, 2, '.', ',') . '</span>';
                })                
                
                ->addColumn('action', '<button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-primary view_payment" data-href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, \'viewPayment\'], [$DT_RowId]) }}">@lang("messages.view")
                    </button> @if(!empty($document))<a href="{{asset("/uploads/documents/" . $document)}}" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-accent" download=""><i class="fa fa-download"></i> @lang("purchase.download_document")</a>@endif')
                ->rawColumns(['invoice_no', 'amount', 'method', 'action', 'customer','base_final_total'])
                ->make(true);
        }
        $business_locations = BusinessLocation::forDropdown($business_id);
    ...
    return view('report.sell_payment_report')
        ->with(compact('business_locations', 'customers', 'payment_types', 'customer_groups', 'currencies'));
        ...
        if (request()->ajax()) {
            ...
            if (! empty(request()->sale_start) && ! empty(request()->sale_end)) {
                ...
                $query->where(function ($q) use ($start, $end): void {
                    $q->where(function ($qr) use ($start, $end): void {
                        $qr->whereDate('sale.transaction_date', '>=', $start)
                            ->whereDate('sale.transaction_date', '<=', $end);
                    })->orWhere(function ($qr) use ($start, $end): void {
            ...
            return Datatables::of($query)
                ...
                ->filterColumn('sale_invoice_no', function ($query, $keyword): void {
    ...
    public function salesRepresentativePrint(Request $request)
    {
        ...
        // Filters
        $user_id = $request->input('sr_id');
        $location_id = $request->input('sr_business_id');
        $date_range = $request->input('date_range');
    
        $start_date = $end_date = null;

if (!empty($date_range)) {
    // Normalize the separator to ' ~ ' just in case
    $normalized_date_range = str_replace(' - ', ' ~ ', $date_range);
    $date_array = explode(' ~ ', $normalized_date_range);

    $start_date = \Carbon\Carbon::createFromFormat('m/d/Y', trim($date_array[0]))->startOfDay();
    $end_date = \Carbon\Carbon::createFromFormat('m/d/Y', trim($date_array[1]))->endOfDay();
}

    
        // Base query for sales
        $sales_query = Transaction::with(['contact', 'location'])
            ->where('business_id', $business_id)
            ->where('type', 'sell')
            ->where('status', 'final');
    
        if (!empty($user_id)) {
            $sales_query->where('created_by', $user_id);
        }
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $sales_query->whereBetween('transaction_date', [$start_date, $end_date]);
        }
        ...
        $payments = TransactionPayment::whereIn('transaction_id', $sales->pluck('id'))->get()->groupBy('transaction_id');
    
        $sales->each(function ($sale) use ($payments) {
            $paid = $payments->get($sale->id)?->sum('amount') ?? 0;
            $sale->total_paid = $paid;
            $sale->total_due = $sale->final_total - $paid;
        });
        ...
        if (!empty($start_date) && !empty($end_date)) {
            $returns_query->whereBetween('transaction_date', [$start_date, $end_date]);
        }
        ...
        if (!empty($user_id)) {
            $expenses_query->where('created_by', $user_id);
        }
    
        if (!empty($location_id)) {
            $expenses_query->where('location_id', $location_id);
        }
    
        if (!empty($start_date) && !empty($end_date)) {
            $expenses_query->whereBetween('transaction_date', [$start_date, $end_date]);
        }
    
        $total_expenses = $expenses_query->sum('final_total');
    
        $net_sales = $total_sales - $total_returns;
    
        // Names
        $user_name = !empty($user_id) ? User::find($user_id)?->user_full_name : __('report.all_users');
        $location_name = !empty($location_id) ? BusinessLocation::find($location_id)?->name : __('report.all_locations');
    
        return view('report.sales_representative_print', compact(
            'sales',
            'total_sales',
            'total_returns',
            'total_expenses',
        ...
        if (request()->ajax()) {
            ...
            if (! empty($subject_type)) {
                if ($subject_type == 'contact') {
                    ...
                    $activities->whereHasMorph('subject', Transaction::class, function ($q) use ($subject_type): void {
            ...
            return Datatables::of($activities)
                ...
                ->filterColumn('created_by', function ($query, $keyword): void {
