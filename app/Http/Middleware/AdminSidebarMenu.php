...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu): void {
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings): void {
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    __('sale.sale'),
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings): void {
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    __('report.reports'),
                    function ($sub) use ($enabled_modules, $is_admin): void {
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getLocationSummary']),
                                __('report.location_summary'),
                                ['icon' => '', 'active' => request()->segment(2) == 'location-summary']
                            );
                        }
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules): void {
                        if (auth()->user()->can('business_settings.access')) {
                            $sub->url(
                        ...
                        $sub->url(
                            url('/currency'), // or use route('currency.index') if defined
                            __('lang_v1.manage_currency_rates'),
                            ['icon' => '', 'active' => request()->segment(1) == 'currency']
                        );
