...
@php
    $fromClass = 'tw-from-' . (session('business.theme_color') ?? 'primary') . '-800';
    $toClass = 'tw-to-' . (session('business.theme_color') ?? 'primary') . '-900';
@endphp
<script type="text/javascript">
    $(document).ready(function() {
        ...
        if ($(window).width() < 1024) {
            $('#custom-bg').addClass("tw-bg-gradient-to-r tw-from-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-800 tw-to-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-900");
        }
        $(window).on('resize', function() {
            ...
            if ($(window).width() < 1024) {
                $('#custom-bg').addClass("tw-bg-gradient-to-r tw-from-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-800 tw-to-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-900");
                $('#custom-bg').removeClass('tw-bg-gray-100');
            }else{
                $('#custom-bg').addClass('tw-bg-gray-100');
                $('#custom-bg').removeClass("tw-bg-gradient-to-r tw-from-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-800 tw-to-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-900");
            }
        ...
        $('.side-bar-collapse').click(function() {
            $('.side-bar').toggle('slow').promise().done(function () {
