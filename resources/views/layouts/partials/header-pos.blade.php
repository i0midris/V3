        ...
        <div class="col-md-6">
            ...
            @if (empty($pos_settings['hide_product_suggestion']))
                <button type="button" title="{{ __('lang_v1.view_products') }}" data-placement="bottom"
                    class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-success m-6 btn-xs btn-modal pull-right tw-block md:tw-hidden"
    ...
    <div
        ...
        <div class="tw-w-full md:tw-w-1/3">
            <div class="tw-flex tw-items-center tw-gap-2">
                <div class="tw-flex flex-items-center form-group tw-gap-1" style="margin-bottom:0 !important">
                    <p class="tw-mt-1"><strong>@lang('sale.location'): </strong></p>
                    @if (empty($transaction->location_id))
                        @if (count($business_locations) > 1)
                            {!! Form::select(
                                'select_location_id',
                ...
                <!-- @if (empty($pos_settings['hide_product_suggestion'])) -->
                    <button type="button" title="{{ __('lang_v1.view_products') }}" data-placement="bottom"
                        class="tw-shadow-sm md:tw-hidden tw-block tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-flex tw-items-center tw-justify-center tw-rounded-md tw-w-8 tw-h-8 tw-text-gray-600 btn-modal pull-right "
                        data-toggle="modal" data-target="#mobile_product_suggestion_modal">
                        <strong><i class="fa fa-cubes fa-lg tw-text-[#00935F] !tw-text-sm"></i></strong>
                    </button>
                <!-- @endif -->
                <div class="tw-hidden md:tw-block !tw-text-xs tw-bg-[#646EE4] hover:tw-bg-[#414aac] tw-py-1.5 tw-px-2 tw-rounded-md">
                    <span class="curr_datetime text-white tw-font-semibold">{{ $now }}</span>
                    <i class="fa fa-keyboard hover-q text-white" aria-hidden="true"
                ...
                <span class="tw-flex md:tw-hidden tw-justify-end tw-flex-1">
                    <i class="fas hamburger fa-bars pull-right"
        ...
        <div class="tw-w-full tw-mx-4 md:tw-w-2/3 !tw-p-0 tw-flex tw-items-center tw-justify-between tw-gap-2 tw-flex-col md:tw-flex-row tw-hidden md:tw-flex"
            id="pos_header_more_options" style="z-index: 1020;">
            <a href="{{ $go_back_url }}" title="{{ __('lang_v1.go_back') }}"
                class="tw-shadow-sm tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-8 tw-w-full tw-h-8 tw-text-gray-600">
                <strong class="!tw-m-3">
                    <i class="fa fa-backward fa-lg tw-text-[#009EE4] !tw-text-sm"></i>
                    <span class="tw-inline md:tw-hidden">{{ __('lang_v1.go_back') }}</span>
                </strong>
            </a>
            <button type="button" title="{{ __('lang_v1.view_products') }}" data-placement="bottom"
                class=" tw-hidden md:tw-flex tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-items-center tw-justify-center tw-rounded-md tw-w-8 tw-h-8 tw-text-gray-600 btn-modal"
                data-toggle="modal" data-target="#mobile_product_suggestion_modal">
                <strong class="!tw-m-3">
                    <i class="fa fa-cubes fa-lg tw-text-[#00935F] !tw-text-sm"></i>
                </strong>
            </button>

            {{-- <a href="{{ $go_back_url }}" title="{{ __('lang_v1.go_back') }}"
              class="md:tw-hidden tw-shadow-sm tw-shadow-sm tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-8 tw-w-full tw-h-8 tw-text-gray-600">
            <strong class="!tw-m-3">
            ...
            @if (!isset($pos_settings['hide_recent_trans']) || $pos_settings['hide_recent_trans'] == 0)
                <button type="button"
                    class="md:tw-hidden tw-shadow-sm tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-8 tw-w-full tw-h-8 tw-text-gray-600"
            ...
            @if (
                ...
                <button type="button"
                    class="tw-shadow-sm tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-8 tw-w-full tw-h-8 tw-text-gray-600 popover-default"
                    id="service_staff_replacement" title="{{ __('restaurant.service_staff_replacement') }}"
                    data-toggle="popover" data-trigger="click"
                    data-content='<div class="m-8"><input type="text" class="form-control" placeholder="@lang('sale.invoice_no')" id="send_for_sell_service_staff_invoice_no"></div><div class="w-100 text-center"><button type="button" class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-error" id="send_for_sercice_staff_replacement">@lang('lang_v1.send')</button></div>'
                    data-html="true" data-placement="bottom">
            ...
            @can('view_cash_register')
                <button type="button" id="register_details" title="{{ __('cash_register.register_details') }}"
                    class="tw-shadow-sm tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-8 tw-w-full tw-h-8 tw-text-gray-600 btn-modal pull-right"
                    data-container=".register_details_modal"
                    data-href="{{ action([\App\Http\Controllers\CashRegisterController::class, 'getRegisterDetails']) }}">
            ...
            <button type="button" id="view_suspended_sales" title="{{ __('lang_v1.view_suspended_sales') }}"
                class="tw-shadow-sm tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-8 tw-w-full tw-h-8 tw-text-gray-600 btn-modal"
                data-container=".view_modal" data-href="{{ $view_suspended_sell_url }}">
                <strong class="!tw-m-3">
            ...
            @can('expense.add')
                <button type="button" title="{{ __('expense.add_expense') }}" data-placement="bottom"
                    class="tw-shadow-sm tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border tw-w-auto tw-h-auto tw-py-1 tw-px-4 tw-rounded-md btn-modal"
                    id="add_expense">
                    <strong><i class="fa fas fa-minus-circle"></i> @lang('expense.add_expense')</strong>
                </button>
            @endcan
