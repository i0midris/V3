<?php
$shipment = (new \App\Utils\ModuleUtil)->getModuleData("Khairataladel_getShipment", ['input'=>['transaction_id' => $receipt_details->transaction_id]]);
if (request()->has('lolo')) {
    dd($receipt_details,$shipment);
}
?>
<style>
	#invoice_content
    ...
    <body style="    background-color: white !important;">
        <div class="ticket">
        	@if(empty($receipt_details->letter_head))
				...
				<div class="text-box">
				<!-- Logo -->
				<p class="centered" style="display: none">
					<!-- Header text -->
			...
			@endif
					<table class="tb border-bottom width-100 table-f-12 mb-10">
						<thead>
						<tr>
							<td class="mybold">رقم الفاتورة</td>
							<td class="mybold">{{$receipt_details->invoice_no ?? ''}}</td>
							<td class="mybold">التاريخ</td>
							<td class="mybold">	{{ $receipt_details->invoice_date ?? ""}}</td>
						</tr>
						</thead>
					</table>
				<style>
            ...
            <table id="myTable" class="tb border-bottom width-100 table-f-12 mb-10">
                <thead class="border-bottom-dotted" border="1">
                    <tr>
                        <td class="serial_number mybold">#</td>
                        <td class="description mybold" width="30%">
                        	المنتج
                        </td>
                        <td class="quantity mybold">
                        	الكمية
                        </td>
                        @if(empty($receipt_details->hide_price))
                        <td class="unit_price mybold">
                        سعر القطعة
                        </td>
                        ...
                        @if(!empty($receipt_details->item_discount_label))
							<td class="mybold">{{$receipt_details->item_discount_label}}</td>
						@endif
                        <td class="price mybold">المجموع</td>
                        @endif
                    </tr>
                </thead>
                <tbody>
                	@forelse($receipt_details->lines as $line)
	                    <tr>
	                        ...
	                        <td class="quantity">{{$line['quantity']}} {{--$line['units']--}} @if($receipt_details->show_base_unit_details && $line['quantity'] && $line['base_unit_multiplier'] !== 1)
                            <br><small>
                            	{{$line['quantity']}} x {{$line['base_unit_multiplier']}} = {{$line['orig_quantity']}} {{$line['base_unit_name']}}
                            </small>
                            @endif</td>
	                        @if(empty($receipt_details->hide_price))
	                        <td class="unit_price">{{$line['unit_price_before_discount']}}</td>
							...
							@endif
	                        <td class="price">{{$line['line_total']}}</td>
	                        @endif
	                    </tr>
				...
				<br>
				@if(isset($shipment['Khairataladel']->code))
						...
						<td style="width: 50%;vertical-align: text-top;">
							<table  id="myTable" style="width: 100%;">
								...
								<tr>
									...
									<td class="mybold">
										{{$shipment['Khairataladel']->tawsil_coast}}
									</td>
								</tr>
								<tr>
									<td class="mybold">
									...
									<td class="mybold">
										{{$shipment['Khairataladel']->mo7afza}}
									</td>
								</tr>
								<tr>
									<td class="mybold">
									...
									<td class="mybold">
										{{$shipment['Khairataladel']->el3nwan}}
									</td>
								</tr>
								<tr>
									<td class="mybold">
								...
								<tr>
									...
									<td class="mybold">
										{{$shipment['Khairataladel']->reciver_phone}}
									</td>
								</tr>
							...
							<table style="width: 100%">
								<tr>
									<td style="    text-align: center;">
										<span style="font-weight: bold">خيرات العادل</span>
										<img class="center-block"
											 src="data:image/png;base64,{{DNS2D::getBarcodePNG(  $shipment['Khairataladel']->code, 'QRCODE', 3,3,array(39, 48, 54), true)}}">
										<span style="font-weight: bold">{{$shipment['Khairataladel']->code}}</span>
									</td>
								</tr>
							</table>
						</td>
