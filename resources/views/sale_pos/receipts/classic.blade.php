...
<html lang="en">
	...
	<body>
		<div class="row" style="color: #000000 !important;">
			<!-- Logo -->
			<div class="col-xs-12 tw-w-full ">
				@if(empty($receipt_details->letter_head))
					<div class="tw-flex tw-flex-col tw-items-center tw-justify-center">
						<!-- Header text -->
						@if(!empty($receipt_details->logo))
							<img style="max-height: 120px; width: auto;" src="{{$receipt_details->logo}}" class="img img-responsive center-bolck">
						@endif
						<p class="tw-font-bold text-center">
			...
			@if(!empty($receipt_details->invoice_heading))
				<h3 class="text-center tw-mt-4 tw-font-bold">
		...
		<div class="row" style="color: #000000 !important;">
			<div class="col-xs-12">
				...
				@if(!empty($receipt_details->discounted_unit_price_label))
					...
					@endphp
				@endif
				<table class="table table-responsive table-slim table-bordered no-footer">
					<thead class="!tw-text-white !tw-bg-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-800">
						<tr>
							...
							<th class="" width="15%">{{$receipt_details->table_qty_label}}</th>
							<th class="" width="15%">{{$receipt_details->table_unit_price_label}}</th>
							@if(!empty($receipt_details->discounted_unit_price_label))
								<th class="" width="10%">{{$receipt_details->discounted_unit_price_label}}</th>
							@endif
							@if(!empty($receipt_details->item_discount_label))
								<th class="" width="10%">{{$receipt_details->item_discount_label}}</th>
							@endif
							<th class="" width="15%">{{$receipt_details->table_subtotal_label}}</th>
						</tr>
					</thead>
					<tbody style="min-height:10rem;">
							...
							@if(!empty($line['modifiers']))
								@foreach($line['modifiers'] as $modifier)
									<tr>
										...
										<td class="">{{$modifier['quantity']}} {{$modifier['units']}} </td>
										<td class="">{{$modifier['unit_price_inc_tax']}}</td>
										@if(!empty($receipt_details->discounted_unit_price_label))
											<td class="">{{$modifier['unit_price_exc_tax']}}</td>
										@endif
										@if(!empty($receipt_details->item_discount_label))
											<td class="">0.00</td>
										@endif
										<td class="">{{$modifier['line_total']}}</td>
									</tr>
								@endforeach
							@endif
						@empty
				...
				<div class="table-responsive">
					<table class="table table-slim no-border">
						<tbody>
							...
							@if (!empty($receipt_details->currency_code) && !empty($receipt_details->exchange_rate) && $receipt_details->exchange_rate != 1)
			<tr>
				<th style="width:70%">
					@lang('lang_v1.currency'):
				</th>
				<td class="">
					{{ $receipt_details->currency_code }}
				</td>
			</tr>
			<tr>
				<th style="width:70%">
					@lang('lang_v1.exchange_rate'):
				</th>
				<td class="">
					{{ @num_format((float) $receipt_details->exchange_rate) }}
				</td>
			</tr>
			<tr>
				<th style="width:70%">
					@lang('lang_v1.total_in_base_currency'):
				</th>
				<td class="">
					{{ @num_format($receipt_details->total_in_base_currency) }}
				</td>
			</tr>
		@endif
		...
		<div class="row" style="color: #000000 !important;margin-top:2rem !important">
