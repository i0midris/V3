...
@endif
    ...
    <tbody>
        @forelse($receipt_details->lines as $line)
            ...
            @if(!empty($receipt_details->currency_code) 
                && !empty($receipt_details->exchange_rate) 
                && $receipt_details->exchange_rate != 1)

                @php
                    $converted_unit = (float) $line['unit_price_before_discount'] * (float) $receipt_details->exchange_rate;
                    $converted_total = (float) $line['line_total'] * (float) $receipt_details->exchange_rate;
                @endphp

                <tr style="border-bottom: 1px dotted #ccc;">
                    <td colspan="6" class="text-right f-8" style="font-style: italic; color: #666;">
                        <span> {{ @num_format($converted_unit) }} {{ $receipt_details->currency_code }}</span> |
                        <span> {{ @num_format($converted_total) }} {{ $receipt_details->currency_code }}</span>
                    </td>
                </tr>
            @endif
        ...
        <tr>
			...
			@if(empty($receipt_details->hide_price))
				...
				@if (!empty($receipt_details->currency_code) && !empty($receipt_details->exchange_rate) && $receipt_details->exchange_rate != 1)
					<div class="flex-box">
						<p class="width-50 text-right">@lang('lang_v1.currency'):</p>
						<p class="width-50 text-right">{{ $receipt_details->currency_code }}</p>
					</div>

					<div class="flex-box">
						<p class="width-50 text-right">@lang('lang_v1.exchange_rate'):</p>
						<p class="width-50 text-right">{{ @num_format((float) $receipt_details->exchange_rate) }}</p>
					</div>

					<div class="flex-box">
						<p class="width-50 text-right">@lang('lang_v1.total_in_base_currency'):</p>
						<p class="width-50 text-right">{{ @num_format($receipt_details->total_in_base_currency) }}</p>
					</div>
				@endif
