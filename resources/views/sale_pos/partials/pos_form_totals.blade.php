	...
	<div class="col-md-12">
        ...
        <td>
          <span class="tw-text-base md:tw-text-lg tw-font-semibold">
            ...
            <span class="tw-text-base md:tw-text-lg tw-font-semibold" id="total_commission">0</span>

            {{-- Hidden fields posted with the form (do NOT affect totals) --}}
            <input type="hidden" name="commission_type" id="commission_type"
                   value="percentage" data-default="percentage">

            {{-- IMPORTANT: leave value empty by default so server can fallback to agent % --}}
            <input type="hidden" name="commission_amount" id="commission_amount"
                   value="" data-default="">

            {{-- Optional marker if you still use it --}}
            <input type="hidden" name="commission_chosen" id="commission_chosen" value="0">

            {{-- Default percent for JS preview; will be synced on agent change --}}
            @php
              $me = auth()->user();
              $default_percent = $default_percent ?? ($me->cmmsn_percent ?? 0);
            @endphp
