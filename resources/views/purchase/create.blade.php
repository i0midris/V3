...
<section class="content">
	...
	@component('components.widget', ['class' => 'box-primary'])
		<div class="row">
			...
			<div class="col-sm-6 col-lg-4 @if(!empty($default_purchase_status)) hide @endif">
				<div class="form-group">
			...
			<div class="col-sm-6 col-lg-4 @if(!$currency_details->purchase_in_diff_currency) hide @endif">
				<div class="form-group">
			...
			<div class="col-sm-6 col-lg-4">
		          <div class="form-group">
		            <div class="multi-input">
		              ...
		              <br/>
		              <div class="input-group">
						{!! Form::number('pay_term_number', null, ['class' => 'form-control input-group-addon pull-left width-40', 'min' => 0, 'placeholder' => __('contact.pay_term')]); !!}
						{!! Form::select('pay_term_type', 
							['months' => __('lang_v1.months'), 
								'days' => __('lang_v1.days')], 
								null, 
							['class' => 'form-control pull-left width-60', 'placeholder' => __('messages.please_select'), 'id' => 'pay_term_type']); 
						!!}
					  </div>
		            </div>
		        </div>
			...
			<div class="col-sm-6 col-lg-4">
                <div class="form-group">
                    ...
                    {!! Form::file('document', ['id' => 'upload_document', 'class'=>'tw-w-full', 'accept' => implode(',', array_keys(config('constants.document_upload_mimes_types')))]); !!}
                    <p class="help-block">
	...
	@component('components.widget', ['class' => 'box-primary'])
		<div class="row">
			...
			<div class="col-sm-5 col-lg-4 tw-flex tw-items-center tw-justify-end tw-gap-2">
				<div>
					<button type="button" class="add-btn" data-toggle="modal" data-target="#import_purchase_products_modal">@lang('product.import_products')</button>
				</div>
				<div>
					<button tabindex="-1" type="button" class="add-btn tw-gap-1 btn-modal"data-href="{{action([\App\Http\Controllers\ProductController::class, 'quickAdd'])}}" 
	...
	@component('components.widget', ['class' => 'box-primary'])
		...
		<div class="row tw-border-b tw-border-gray-300 tw-py-3">
			<div class="col-sm-3">
				<div class="form-group">
	...
	<div class="row">
		...
		<div class="row">
			<div class="col-md-12">
				<button type="button" class="add-btn tw-gap-2" id="toggle_additional_expense"> <i class="fas fa-plus"></i> @lang('lang_v1.add_additional_expenses') <i class="fas fa-chevron-down"></i></button>
			</div>
	...
	@endcomponent
	@component('components.widget', ['class' => 'box-primary hide', 'title' => __('purchase.add_payment')])
...
@section('javascript')
	...
	$(document).ready(function() {
		...
		// 🔽 Your custom feature: calculate cost fields based on line_total input
		$(document).on('change', '.row_subtotal_after_tax_hidden', function () {
		    const row = $(this).closest('tr');
		    let line_total = __read_number($(this), true);
		    let quantity = __read_number(row.find('.purchase_quantity'), true);

		    if (!quantity || quantity === 0) {
		        quantity = 1;
		        __write_number(row.find('.purchase_quantity'), quantity, true);
		    }

		    let unit_cost_after_tax = line_total / quantity;

		    let tax_rate = parseFloat(row.find('select.purchase_line_tax_id option:selected').data('tax_amount')) || 0;
		    let unit_cost_before_tax = __get_principle(unit_cost_after_tax, tax_rate);

		    let discount_percent = __read_number(row.find('.inline_discounts'), true) || 0;
		    let unit_cost_without_discount = __get_principle(unit_cost_before_tax, discount_percent, true);

		    __write_number(row.find('.purchase_unit_cost_after_tax'), unit_cost_after_tax, true);
		    __write_number(row.find('.purchase_unit_cost'), unit_cost_before_tax, true);
		    __write_number(row.find('.purchase_unit_cost_without_discount'), unit_cost_without_discount, true);

		    let tax_amount = unit_cost_after_tax - unit_cost_before_tax;
		    row.find('.purchase_product_unit_tax_text').text(__currency_trans_from_en(tax_amount, false, true));
		    __write_number(row.find('.purchase_product_unit_tax'), tax_amount, true);

		    row.find('.row_subtotal_after_tax').text(__currency_trans_from_en(line_total, false, true));
		    __write_number(row.find('.row_subtotal_after_tax_hidden'), line_total, true);

		    let subtotal_before_tax = unit_cost_before_tax * quantity;
		    row.find('.row_subtotal_before_tax').text(__currency_trans_from_en(subtotal_before_tax, false, true));
