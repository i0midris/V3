...
include_once __DIR__.'/install_r.php';

Route::middleware(['setData'])->group(function (): void {
...
Route::middleware(['auth'])->group(function (): void {
    // 🔄 Run the system update (via AJAX)
    Route::post('/admin/system-update', [SystemUpdateController::class, 'run'])
        ->name('system.update');

    // 🆕 Show "What's New" changelog from GitHub
    Route::get('/admin/system-whats-new', [SystemUpdateController::class, 'whatsNew'])
        ->name('system.whats_new');
});

// optimize and cache clear
Route::post('/clear-optimize', function () {
    Artisan::call('optimize:clear');

    return response()->json(['message' => 'Optimize cleared successfully']);
})->middleware('auth')->name('clear.optimize');
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function (): void {
...
// Currency routes
Route::get('/currency', [CurrencyRateController::class, 'index'])->name('currency.index');
Route::post('/currency/store', [CurrencyRateController::class, 'store'])->name('currency.store');
Route::post('/currency/update/{id}', [CurrencyRateController::class, 'update'])->name('currency.update'); // POST instead of PUT
Route::get('/currency/delete/{id}', [CurrencyRateController::class, 'destroy'])->name('currency.destroy'); // GET instead of DELETE
Route::get('/currency/edit/{id}', [CurrencyRateController::class, 'edit'])->name('currency.edit');
Route::get('/currency-rates/get-rate/{code}', [CurrencyRateController::class, 'getRate']);
    ...
    Route::get('/products/lookup', [SellPosController::class, 'lookupByCode']);
    ...
    Route::get('/currencies/get-rate/{code}', [\App\Http\Controllers\CurrencyController::class, 'getExchangeRate']);
    Route::get('/currencies/active', [\App\Http\Controllers\CurrencyController::class, 'getActiveCurrencies']);
...
Route::get('/pos/commission-agents/{user}', [\App\Http\Controllers\SellPosController::class, 'getAgentCommissionPercent'])
    ...
    // Route::get('pos/{id}/edit'[SellPosController::class,edit)->name('pos.edit');
    ...
    Route::get('/reports/location-summary', [ReportController::class, 'getLocationSummary'])->name('reports.location_summary');
    ...
    Route::prefix('business-location/{location_id}')->name('location.')->group(function (): void {
    ...
    Route::get('reports/sales-representative-print', [ReportController::class, 'salesRepresentativePrint'])
    ->name('reports.sales_representative.print');
    ...
    Route::prefix('modules')->group(function (): void {
        ...
        Route::get('/product-modifiers/{id}/edit', [ProductModifierSetController::class, 'edit']);
        Route::post('/product-modifiers/{id}/update', [ProductModifierSetController::class, 'update']);
        Route::get('/product-modifiers/product-row/{product_id}', [ProductModifierSetController::class, 'product_row']);

        Route::get('/add-selected-modifiers', [ProductModifierSetController::class, 'add_selected_modifiers']);

        Route::get('/kitchen', [KitchenController::class, 'index']);
        Route::get('/kitchen/mark-as-cooked/{id}', [KitchenController::class, 'markAsCooked']);
        Route::post('/refresh-orders-list', [KitchenController::class, 'refreshOrdersList']);
        Route::post('/refresh-line-orders-list', [KitchenController::class, 'refreshLineOrdersList']);

        Route::get('/orders', [OrderController::class, 'index']);
        Route::get('/orders/mark-as-served/{id}', [OrderController::class, 'markAsServed']);
        Route::get('/data/get-pos-details', [DataController::class, 'getPosDetails']);
        Route::get('/data/check-staff-pin', [DataController::class, 'checkStaffPin']);
        Route::get('/orders/mark-line-order-as-served/{id}', [OrderController::class, 'markLineOrderAsServed']);
        Route::get('/print-line-order', [OrderController::class, 'printLineOrder']);
    });
    ...
    Route::resource('manage-modules', ModulesController::class)
        ->only(['index', 'update']);
    Route::get('regenerate', [ModulesController::class, 'regenerate']);
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone'])->group(function (): void {
